...
- [x] phase5-evmethod-knowledge-0.0.34 -- #171 EVM 知識ベース化（docs/EVM-KNOWLEDGE.md）を実施。evMethod（0/100,50/50）は一旦先送り後、**2026-07-06 に実装へ格上げ・実装完了**（EV測定技法は PMI 標準のドメイン計算で利用側合成不可のため）。機能化候補は #184/#185/#186 に起票。Dependencies: phase3-earned-schedule-0.0.32
- [ ] schedule-adherence -- Lipke の P-Factor（Schedule Adherence）。SPI(t) 良好でも隠れる先食い（手戻りリスク）を検出。**spec 生成済み・実装はトリガー待ち**（利用側スキルの plan-adherence 定着。2026-07-07 spec化）。Dependencies: phase3-earned-schedule-0.0.32, phase5-evmethod-knowledge-0.0.34
