...
| 項目 | 内容 |
|------|------|
| 計算式 | 基準日の翌日〜終了日でplotMapがtrueの日数 |
| 条件 | 基準日がタスク期間外の場合は0 |
| 備考 | 基準日はExcelデータの「その日終了時点」を表すため、基準日自体は含まない |
...
| AC-ID | 受け入れ基準 |
|-------|-------------|
| AC-01 | `TaskRow.remainingDays` で基準日の翌日〜終了日の残日数を取得できる |
...
| 1.0.1 | 2026-01-28 | remainingDays定義修正: 基準日を含まない（基準日終了時点の解釈）#156 | Claude Code |
