...
サブパス export（`evmtools-node/{domain,infrastructure,usecase,common,logger}`）のバレルに載る全シンボル。
`resource` はベータ（API 安定化前）のため安定化までカタログ対象外。
...
| シンボル | 種別 | 概要 | 導入 | 詳細 |
|----------|------|------|------|------|
| `Project` | class | 集約ルート。`getStatistics` / `getStatisticsByName` / `getDelayedTasks` / `getTree` / **`getDailyPvByAssignee`(0.0.30)** / **`calculateEarnedSchedule`(0.0.31)** / `calculateCompletionForecast` / `calculateRecentDailyPv` / `filterTasks` / `excludedTasks` / PV 系 getter 等 | - | Project.spec.md |
...
| `ProjectCreator` / `TaskRowCreator` / `ProjectRepository` / `ProjectStatisticsCreator` | interface | 生成・永続化の抽象 | - | ProjectCreator.spec.md / Interfaces.spec.md |
...
| `DailyPvEntry` / `DailyPvTaskDetail` / `DailyPvByAssigneeOptions` | interface | 日次PV担当者別集計（明細付き） | 0.0.30 | Project.spec.md |
| `EarnedScheduleResult` | interface | Earned Schedule 系指標（ES/AT/SPI(t)/SV(t)/IEAC(t)/PD） | 0.0.31 | Project.spec.md 5.18 |
| `EvMethod` | type | EV 算定方式（'progressRate'/'0/100'/'50/50'）。StatisticsOptions.evMethod で指定 | 0.0.33 | Project.spec.md 5.19 |
...
| `convertToTaskRow` / `checkTaskName` | function | DTO 変換・検査 | - | - |
...
| 2026-07-04 | phase1-minor-issues-0.0.30: 公開 API 追加なし（getFullTaskName の内部メモ化のみ。#166/#138/#165 の公開 API 追加は公開 API 追加基準の適用によりスコープ外し） |
| 2026-07-04 | phase2-skill-integration-0.0.31（縮小版）: Project.getDailyPvByAssignee + 関連型3つを追加（日次PV計算の単一ソース化）。AlertService / detectActiveSubprojects は基準適用で取り下げ |
| 2026-07-04 | phase3-earned-schedule（縮小版）: Project.calculateEarnedSchedule + EarnedScheduleResult を追加（ES/SPI(t)。Statistics 統合は基準適用で取り下げ） |
| 2026-07-06 | phase5-evmethod: StatisticsOptions.evMethod（EV 算定方式 0/100・50/50）+ EvMethod 型を追加（格上げ判断による実装） |
