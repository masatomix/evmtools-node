...
| シンボル | 種別 | 概要 | 導入 | 詳細 |
|----------|------|------|------|------|
| `Project` | class | 集約ルート。`getStatistics` / `getStatisticsByName` / `getDelayedTasks` / **`getIncompleteTasksUpToToday`(0.0.30)** / `getTree` / **`getNameWithGreeting`(0.0.30)** / `calculateCompletionForecast` / `calculateRecentDailyPv` / `filterTasks` / `excludedTasks` / PV 系 getter 等 | - | Project.spec.md |
| `TaskRow` | class | `calculatePV(s)` / `calculateSPI` / `calculateSV` / `remainingDays` / `pvTodayActual` / `isOverdueAt` / `finished` / **`fullName`・`setFullName`(0.0.30)** 等 | - | TaskRow.spec.md |
...
| `TaskDiff` / `TaskDiffBase` / `ProjectDiff` / `AssigneeDiff` / `DiffType` | type | スナップショット差分（TaskDiff.isReschedule は 0.0.30〜） | - | ProjectService.spec.md |
...
| 2026-07-04 | phase1-minor-issues-0.0.30: Project に getNameWithGreeting / getIncompleteTasksUpToToday、TaskRow に fullName/setFullName、TaskDiff に isReschedule を追記 |
