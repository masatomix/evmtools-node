...
| シンボル | 種別 | 概要 | 導入 | 詳細 |
|----------|------|------|------|------|
| `Project` | class | 集約ルート。`getStatistics` / `getStatisticsByName` / `getDelayedTasks` / `getTree` / **`getDailyPvByAssignee`(0.0.30)** / `calculateCompletionForecast` / `calculateRecentDailyPv` / `filterTasks` / `excludedTasks` / PV 系 getter 等 | - | Project.spec.md |
