...
| 実装 | ✅ | 2026-01-27 |
| 統合テスト | ✅ | 2026-01-27 |
| トレーサビリティ更新 | ✅ | 2026-01-27 |
| マスター設計書反映 | ✅ | 2026-01-27 |
...
- [x] **T-04**: テストファイル作成 (`ProjectService.recent-spi.test.ts`)
  - TC-01〜TC-04: 正常系テスト
  - TC-05〜TC-08: 境界値テスト
  - TC-09〜TC-12: 警告テスト
...
- [x] **T-05**: `RecentSpiOptions` 型定義を追加
- [x] **T-06**: `calculateRecentSpi()` メソッド実装
- [x] **T-07**: `_warnIfPeriodTooLong()` 内部メソッド実装
- [x] **T-08**: エクスポート追加（`src/domain/index.ts` - 既存のre-exportで対応）

### Phase 4: 検証 ✅

- [x] **T-09**: 全テスト実行・PASS確認（12件）
- [x] **T-10**: 既存テスト（223件）がPASSすることを確認（合計235件PASS）
- [x] **T-11**: lint / format チェック

### Phase 5: ドキュメント更新 ✅

- [x] **T-12**: 要件トレーサビリティ更新（設計書）
- [x] **T-13**: マスター設計書への反映（`ProjectService.spec.md` 更新）
...
|------|---------|
| 2026-01-27 | 初版作成 |
| 2026-01-27 | Phase 2〜4 完了（テスト・実装・検証） |
| 2026-01-27 | Phase 5 完了（マスター設計書反映） |
