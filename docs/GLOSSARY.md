...
| 工期 | Duration | **カレンダー日数**。開始日から終了日までの暦上の日数。遅延日数の計算に使用 |
| 工数 | Effort / Man-days | **作業量**。人日単位の作業量。PV/EV計算に使用 |

> **工期と工数の違い**:
> - **工期**: 「いつからいつまで」という期間（カレンダー日数）。遅延判定は工期ベースで行う
> - **工数**: 「何人日かかるか」という作業量。EVM（PV/EV）計算は工数ベースで行う
...
#### Earned Schedule 系指標（0.0.31〜、`Project.calculateEarnedSchedule()`）

古典 SPI/SV は「量」の比較のため、プロジェクト終盤で SPI が必ず 1.0 に収束し遅延が見えなくなる欠点がある（知見ⓑ）。Earned Schedule（Lipke / PMI）は「時間」で測ることでこれを解消する。

| 指標 | 日本語 | 計算式 | 意味 |
|------|--------|--------|------|
| **ES** | アーンドスケジュール | 累積PV曲線上で「現在のEVに計画が到達した時点」（稼働日、線形補間） | 出来高を時間に換算した値 |
| **AT** | 実際経過時間 | 開始日〜基準日の稼働日数 | Actual Time |
| **SPI(t)** | 時間ベースのスケジュール効率 | ES / AT | 1.0未満なら時間的に遅延。終盤でも1.0に収束しない |
| **SV(t)** | 時間ベースのスケジュール差異 | ES − AT | 「何稼働日遅れているか」を直接表す |
| **PD** | 計画期間 | プロジェクト全期間の稼働日数 | Planned Duration |
| **IEAC(t)** | 時間ベースの完了予測 | PD / SPI(t) | 現在の時間効率が続いた場合の総所要稼働日数（`esForecastDate` で暦日化） |

#### EV 算定方式（evMethod、0.0.33〜）

`StatisticsOptions.evMethod` で EV の算定方式を選べる（既定は従来どおり進捗率按分）:

| 方式 | EV の式 | 特徴 |
|------|---------|------|
| `'progressRate'`（既定） | 進捗率 × 工数 | 従来どおり。進捗率は PM の主観入力 |
| `'0/100'` | 完了時のみ工数を計上 | 最も保守的・客観的（PMI fixed formula）。仕掛かりが多いと EV が小さく出る |
| `'50/50'` | 着手（actualStartDate あり）で半分、完了で残り | 中間的な客観方式 |

> **注意（50/50 の罠）**: 着手判定は **actualStartDate の有無**で行う（進捗率を使うと主観が再混入するため意図的）。
> 進捗率が入っていても actualStartDate が未入力のタスクは EV=0 になる。50/50 を使う場合は実績開始日の入力運用が前提。
