...
### 3.2 仕様駆動開発（Spec-Driven Development）

#### 3.2.1 Forward フロー（新規開発）
...
**ポイント**:
- 1.1.0: 機能追加なので**新規要件ID（REQ-CSV-002）を払い出し**（3.2.6参照）
...
#### 3.2.7 開発中の修正フロー（同一案件の手戻り）

> **注意**: このセクションは**同一案件の開発中**に気づいた設計ミスの修正フローです。
> **既にリリースされた機能への追加・拡張**は、新規要件ID払い出し（3.2.6参照）で対応してください。
...
#### 3.2.8 GitHub Issue連携フロー

##### 3.2.8.1 基本方針: Issue First

GitHub IssueとPRを要件定義書・仕様書と連携させ、開発プロセスを効率化する。
...
| 成果物 | 役割 | 特徴 |
|--------|------|------|
| **GitHub Issue** | 起点・議論の場 | 背景・動機・変更理由を記録、コメントで議論 |
| **要件定義書** | 確定した仕様 | Issueの議論を経て「決まったこと」を記録 |
| **Pull Request** | 実装の成果物 | `Closes #nn` でIssueと紐付け、レビュー |
...
1. Issue作成
   ...
   │
   ▼
2. 要件定義書作成（REQ-xxx.md）
   │  - GitHub Issue欄に #nn を記載
   │  - Issueの議論を踏まえて仕様を確定
   │
   ▼
3. 仕様書 → テスト → 実装（従来通り）
...
| 項目 | 効果 |
|------|------|
| **進捗管理** | IssueのOpen/Closedで一目瞭然 |
| **トレーサビリティ** | Issue → 要件定義書 → 仕様書 → PR → コミット |
| **議論の記録** | Issueコメントで変更理由を残せる |
| **Project連携** | GitHub Projectsでカンバン管理可能 |
| **検索性** | Issue番号で関連する全成果物を追跡可能 |
